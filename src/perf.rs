...
impl PerfLogger {
    ...
    fn start_run(&self) -> Result<(), String> {
        ...
        let started_at = self
            .started_at
            .as_deref()
            .ok_or_else(|| "Missing perf log start date".to_string())?;
    ...
    fn append(&self, content: &str) -> Result<(), String> {
        OpenOptions::new()
            ...
            .map_err(|error| format!("Could not write perf log: {error}"))
    }
}

fn formatted_now() -> Result<String, String> {
    let output = Command::new("date")
        .arg("+%d/%m/%Y %H:%M:%S")
        .output()
        .map_err(|error| format!("Could not format perf log date: {error}"))?;

    if !output.status.success() {
        return Err("Could not format perf log date".to_string());
    }

    String::from_utf8(output.stdout)
        .map(|output| output.trim().to_string())
        .map_err(|error| format!("Invalid perf log date output: {error}"))
}

fn perf_log_path() -> Result<&'static Path, String> {
    let path = Path::new(PERF_LOG_PATH);

    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("Could not create perf log directory: {error}"))?;
    }
